...
class ERPDiffusion_0_1_3(ERPDiffusion_0_1_0):
    """ ERPDiffusion_0_1_0 + weaken mask proportional to phi level
    """
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            for i, t in enumerate(tqdm(self.scheduler.timesteps)):
                ...
                for j, w_j in enumerate(w_js):
                    ...
                    phi_tensor = torch.tensor([self.views[j][1]], dtype=torch.float32, device=self.device)
                    
                    # Cos phi mask
                    phi_rad = phi_tensor * torch.pi / 180
                    mask_phi = torch.cos(phi_rad) * mask_x_j
                    
                    # # Linear phi mask
                    # phi_linear = (90 - torch.abs(phi_tensor)) / 90
                    # mask_phi = phi_linear * mask_x_j
                    
                    value[:, :] += x_erp_up_j * mask_phi
                    count[:, :] += mask_phi

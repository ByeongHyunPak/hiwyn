...
class ERPDiffusion_0_1_6(ERPDiffusion_0_1_0):
    """ ERPDiffusion_0_1_1 (fusion w/ average) -> randomly select on overlap region
    """
    ...
    def text2erp(self,
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps)):
            ...
            for j, w_j in enumerate(w_js):
                ...
                value.append(x_erp_up_j * mask_x_j)
                count[:, :] += mask_x_j
            ...
            value = torch.stack(value, dim=0) # [num_views, 1, C, H, W]
            valid_counts = count[0, 0].long()
            random_indices = torch.randint(0, valid_counts = count[0, 0].long().max().item(), size=valid_counts.shape, device=self.device)
            random_indices_expanded = random_indices[None, None, None, :, :]  # Expand to [1, 1, 1, H, W]
            x_erp_up = torch.gather(
                value, 0, random_indices_expanded.expand(value.shape[0], 1, value.shape[2], *random_indices.shape)
